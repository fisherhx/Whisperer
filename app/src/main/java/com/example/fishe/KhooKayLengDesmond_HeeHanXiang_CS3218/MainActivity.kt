...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sendButton?.setOnClickListener {
            ...
            for(index in msgToSend.indices){
                ...
                //Convert Capital A,B,C to ascii values 123-125
    ...
    private fun transmitMessage(byteArray: ByteArray?) {
        ...
        for(i in 0..size-1) {
            soundTransmitter.playSound(freqResolution * 205, 0.2)
            soundTransmitter.playSound(freqResolution * (offset + 2 * (byteArray?.get(i)!!.toInt() - 32)), 0.3)
        }
